...
class CommentUser(AsyncConsumer):
    ...
    async def websocket_receive(self, event):
        ...
        if front_text is not None:
            ...
            print('created_review', created_review)
    ...
    def create_comment(self, task_slug, user, review):
        global created_review
        task=Tasks.objects.get(slug=task_slug)
        if not check_write_comment(task, user):
            raise PermissionError
        else:
            created_review = TaskReviews.objects.create(task=task, user=user, review=review)
            return created_review


class ChangeCommentUser(AsyncConsumer):
    async def websocket_connect(self, event):
        print("connected", event)
        await self.send({
            "type": "websocket.accept"
